"""End-to-end test for the DINOv3 feature-extraction pipeline.
...
production path for one minimal example:
    ...
    3. run extraction normally (extract_dinov3_features) -> feature CSV
...
Requires a GPU node with the DINOv3 checkpoint on disk.

Run with:
    uv run python tests/e2e_tests/dinov3_e2e.py
"""
...
CHANNEL = "Phase2D"
EXPECTED_DIM = 1024  # DINOv3 ViT-L/16 embedding width
...
def build_config(link_dir: Path, output_dir: Path) -> dict:
    """Inline config; mirrors configs/model_configs/dinov3/ops0031_dino.yml."""
    return {
        "model_type": "dinov3",
...
def main() -> None:
    tmp = Path(tempfile.mkdtemp(prefix="dinov3_e2e_"))
    ...
    # 3. run extraction
    extract_dinov3_features(config=config)
    feature_csv = output_dir / f"dinov3_features_{CHANNEL}.csv"
    ...
    print(f"\n✓ DINOv3 e2e PASSED. Outputs under: {tmp}")
