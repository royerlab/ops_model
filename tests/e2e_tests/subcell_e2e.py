"""End-to-end test for the SubCell feature-extraction pipeline.
...
production path for one minimal example:
    ...
    3. run extraction normally (extract_subcell_features) -> feature CSV
    4. convert to AnnData (process_features_csv) -> cell/guide/gene .h5ad
    5. verify the outputs at each step

SubCell is a two-channel (DNA + protein) model: out_channels is
[dna_channel, protein_channel] and the embedding is named after the protein
channel. Crops are read from the real phenotyping_v3.zarr at the subsampled
bboxes, so only the link CSV is subset. All outputs go to a fresh tmp dir
(printed at the end for inspection).

Requires a GPU node; the SubCell weights download from S3 on first use and cache
under the model_checkpoints dir.

Run with:
    uv run python tests/e2e_tests/subcell_e2e.py
"""
...
DNA_CHANNEL = "nuclei_prediction"  # SubCell bg model's nuclear/Blue channel
PROTEIN_CHANNEL = "GFP"  # protein-of-interest; embedding is named after this
OUT_CHANNELS = [DNA_CHANNEL, PROTEIN_CHANNEL]  # [DNA, protein]
EXPECTED_DIM = 1536  # SubCell gated-attention pooler (2 heads x 768)
...
def build_config(link_dir: Path, output_dir: Path) -> dict:
    """Inline config; mirrors experiments/embedding/configs/subcell/ops0031_subcell.yml.

    Note: extract_subcell_features consumes out_channels directly as
    [dna_channel, protein_channel] (the orchestrator's dna_channel pairing is
    inlined here).
    """
    return {
        "model_type": "subcell",
        "embedding_type": "subcell",
        "dataset_type": "basic",
        "data_manager": {
...
def main() -> None:
    tmp = Path(tempfile.mkdtemp(prefix="subcell_e2e_"))
    ...
    # 3. run extraction (output is named after the protein channel)
    extract_subcell_features(config=config)
    feature_csv = output_dir / f"subcell_features_{PROTEIN_CHANNEL}.csv"
    ...
    print(f"\n✓ SubCell e2e PASSED. Outputs under: {tmp}")
