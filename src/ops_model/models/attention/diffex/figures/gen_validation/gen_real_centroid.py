...
OUT = "/hpc/projects/icd.fast.ops/analysis/figure4_traversals/gen_real_centroid"
...
def embed_centroids(grain, classes):
    from ops_model.models.attention.diffex.viewer.precompute import _gather_class
...
def score(grain):
    ...
    names = list(d["names"]); cidx = {c: i for i, c in enumerate(names)}
    mu_r, sd_r = d["mu"], d["sd"]
    ...
    g0 = []
    for f in caches:
        dd = np.load(f, allow_pickle=True); z0 = dd["gen"][len(dd["alphas"]) // 2]
    ...
    json.dump({"alphas": sorted(by), "by_alpha": by, "n_classes": len(cidx)},
              open(f"{OUT}/{grain}_scored.json", "w"))
...
def ceiling(grain):
    ...
    for f in glob.glob(f"{CACHE}/{grain}/*.npz"):
        ...
        if c not in cidx:
            continue
        ti = cidx[c]; rz = (np.asarray(dd["real"], np.float32) - d["mu"]) / d["sd"]
...
def plot(min_dist=None, fname="centroid_topk"):
    """3 scores (mAP, top-1, top-5) vs α from the faithful centroids. min_dist: restrict to classes whose real
    distinctiveness/EBI mAP@20 (gen_real_distinct/{grain}_real.json) > min_dist."""
    import matplotlib
    matplotlib.use("Agg")
    ...
    for ax, (grain, lbl) in zip(axes, [("geneKO", "Gene-level"), ("complex", "Protein complex")]):
        ...
        def vals(a, key):
            dd = ba[str(a)][key]
            return [v for c, v in dd.items() if keep is None or c in keep]
        ...
        for key, col, name in [("map", CM, "MRR"), ("top1", C1, "top-1"), ("top5", C5, "top-5")]:
            ...
            ax.plot(al, m, "-", color=col, lw=2.6, label=f"{name} — generated")
            ...
            if ceil:                                                     # dotted = real-cell ceiling (same centroids)
                cvs = [v[key] for c, v in ceil.items() if keep is None or c in keep]
    ...
    axes[0].set_ylabel(f"generated → nearest real centroid (top-{N} cells)"); axes[0].set_ylim(-0.02, 1.02); axes[0].legend(fontsize=11)
