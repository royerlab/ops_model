...
const METHODS_SLIDES = [
  ...
  },
  {
    ...
    body: "Each cell is stored as two separate parts — its <b>identity</b> (size, texture, local context) and its <b>phenotype</b> (the CellDINO semantic code). We hold the identity <b>fixed</b> and move only the phenotype, sliding the semantic code along the <b>NTC → knockout direction</b> (the average difference between control and knockout codes) and decoding each step. α = 0 is the real cell, α = 1 the full knockout shift, and beyond exaggerates it. Because identity stays locked, the same cell's size, texture, and surroundings are preserved — only the phenotype changes.",
    why: "You watch one real cell change — its context held constant, only the perturbation's phenotype altered — instead of a slideshow of different cells, so every difference you see is the phenotype itself.",
