...
_directions tree, so no direction re-fit — only the 200-anchor inversion + 200×7 decodes per target.
  ...
  python -m ops_model.models.attention.diffex.viewer._build_valid200 anchor    # 1 GPU: build the 200-cell NTC anchor
  python -m ops_model.models.attention.diffex.viewer._build_valid200 submit     # shard all 1000 geneKO (after anchor)
  python -m ops_model.models.attention.diffex.viewer._build_valid200 all        # anchor job -> shards (afterok dep)
"""
import os
import sys
