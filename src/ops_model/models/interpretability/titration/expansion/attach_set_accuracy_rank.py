"""Attach set_accuracy_rank + Alex's authoritative gene name for cells he
ranked (per-gene / geneKO version).

The CSV/h5ad gene-name mismatch (NTC cells reassigned, barcode-mapping drift)
makes joining by (well, seg) alone unsafe if we then group by obs.perturbation.
We attach Alex's gene_name as ``set_accuracy_gene`` and let the sweep use that
as the grouping key for set_accuracy selections. All ranked cells retained.
"""
...
CSV = Path(
    ...
    "accuracy_ranking/pergene_phase_cell_rankings.csv"
)
PCA_DIR = Path(
    "/hpc/projects/icd.fast.ops/models/alex_lin_attention/v4/"
...
def main() -> None:
    ...
    # Dedup: keep (gene_name, best rank) per (exp, well, seg)
    dedup = (
    ...
    for exp in all_exps:
        ...
        a.obs["set_accuracy_rank"] = rank_vals
        rank_type = np.where(np.isfinite(rank_vals), "top", "")
        a.obs["set_accuracy_rank_type"] = pd.Categorical(rank_type)
        a.obs["set_accuracy_gene"] = pd.Categorical(alex_gene)
        a.write_h5ad(p)
