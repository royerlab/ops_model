...
V4 = Path("/hpc/projects/icd.fast.ops/models/alex_lin_attention/v5")
import os as _os
_EXP_SUBDIR = _os.environ.get("V5_EXPANSION_SUBDIR", "expansion_v1")
EXPANSION = V4 / _EXP_SUBDIR
PCA_DIR = EXPANSION / "per_experiment_v5_pca"
...
def _random_k_uncovered(obs: pd.DataFrame, rank_col: str, K: int,
                          group_col: str = "perturbation",
                          seed: int = 0) -> np.ndarray:
    """Random K cells per uncovered gene (obs.perturbation for the grouping,
    since uncovered genes lack a group_col value). Used to give ebionly a fair
    K-cells-per-gene apples-to-apples comparison across all 1000 genes."""
    obs = obs.reset_index(drop=True)
    unc_pos = _uncovered_gene_cells(obs, rank_col, group_col=group_col)
    if unc_pos.size == 0:
        return np.array([], dtype=np.int64)
    # Group the uncovered cells by their h5ad-side perturbation (Alex didn't
    # rank them, so ``group_col`` is empty for these — fall back to obs.pert
    # which reflects the actual gene identity from sgrna_lookup).
    unc_pert = obs["perturbation"].to_numpy()[unc_pos]
    rng = np.random.default_rng(seed)
    parts = []
    for g in np.unique(unc_pert):
        positions = unc_pos[unc_pert == g]
...
def _select_top_per_gene(obs: pd.DataFrame, rank_col: str, K: int,
                          ...
                          uncovered_mode: str = "all",
                          seed: int = 0) -> np.ndarray:
    """Top-K cells per gene for head-covered genes.

    ``include_uncovered`` toggles whether cells of genes outside the head's
    panel contribute to the bin. ``uncovered_mode`` controls HOW they contribute:
    - "all"      : ALL cells of each uncovered gene (curve saturates to
                   all-cells baseline at high K). Historical default.
    - "random_k" : Random K cells per uncovered gene (matches the K used for
                   covered genes → fair apples-to-apples mAP across 1000 genes).
    """
    obs = obs.reset_index(drop=True)
    ...
    if include_uncovered:
        if uncovered_mode == "random_k":
            uncovered_idx = _random_k_uncovered(
                obs, rank_col, K, group_col=group_col, seed=seed)
        else:
            uncovered_idx = _uncovered_gene_cells(
                obs, rank_col, group_col=group_col)
...
def _select_bottom_per_gene(obs: pd.DataFrame, rank_col: str, K: int,
    ...
    for ``include_uncovered`` / ``uncovered_mode`` semantics.
    """
    obs = obs.reset_index(drop=True)
...
def _select_for_bin(obs: pd.DataFrame, bin_id: str, seed: int = 0) -> np.ndarray:
    ...
    if meta["direction"] == "random":
        ...
        # v5 heads: group by Alex's authoritative gene name (attached alongside
        # the rank). Coverage per head:
        # - gko:      1000 real genes → uncovered fallback OFF (no dilution)
        # - ebionly:  311 EBI-panel genes → uncovered ON with random-K per
        #             uncovered gene → fair K-per-gene apples-to-apples
        #             mAP across all 1000 genes (not "all cells" padding).
        # - ebifb:    1000 real genes → OFF (same reason as gko)
        group_col = f"{meta['head']}_gene"
        include_uncovered = (meta["head"] == "ebionly")
        uncovered_mode = "random_k" if meta["head"] == "ebionly" else "all"
        if meta["direction"] == "top":
            sel = _select_top_per_gene(obs, rank_col, meta["K"],
                                        group_col=group_col,
                                        include_uncovered=include_uncovered,
                                        uncovered_mode=uncovered_mode,
                                        seed=seed)
        else:
...
def build_bin_guide_means(bin_id: str, guide_dir: Path = GUIDE_DIR) -> Optional[Path]:
    ...
    # ---- Pass A: global obs table (v5-only heads) ----
    obs_cols = ["perturbation", "sgRNA",
                "gko_rank",     "gko_rank_type",     "gko_gene",
                "ebionly_rank", "ebionly_rank_type", "ebionly_gene",
                "ebifb_rank",   "ebifb_rank_type",   "ebifb_gene"]
    obs_blocks = []
