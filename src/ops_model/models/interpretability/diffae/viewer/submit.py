...
one-off scratchpad drivers). All target selection comes from `catalog.py`.
  ...
  python -m ops_model.models.interpretability.diffae.viewer.submit seed              # per-marker NTC traversals
  python -m ops_model.models.interpretability.diffae.viewer.submit anchors --k 5     # A→B anchor pairs
  python -m ops_model.models.interpretability.diffae.viewer.submit manifest          # rebuild manifest.json (local)
  python -m ops_model.models.interpretability.diffae.viewer.submit montage --cell 0 --alpha 2   # harvest cache -> UMAP montage zarr
"""
