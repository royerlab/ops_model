...
const MTH_REFS = {
  ...
  "Top-Predictive Cells": [["Explaining by removing · Covert 2021", "https://arxiv.org/abs/2011.14878"], ["SHAP · Lundberg & Lee 2017", "https://arxiv.org/abs/1705.07874"]],
  ...
  "Image Generation": [["DiffEx · Bourou 2025", "https://arxiv.org/abs/2502.09663"], ["Diffusion autoencoders · Preechakul 2022", "https://arxiv.org/abs/2111.15640"], ["Classifier-free guidance · Ho & Salimans 2022", "https://arxiv.org/abs/2207.12598"]],
...
const MTH_MORE = {
  ...
  "Classifier": "A single knocked-out cell is often ambiguous — cells vary a lot even with no perturbation — so the signal lives in the distribution of a perturbation's cells. We therefore classify a whole bag at once (multiple-instance learning). The SetTransformer tags each cell's fingerprint with its imaging channel, uses attention so the cells in a bag can inform one another, then pools them into a single bag-vector with attention (PMA) rather than a plain average, letting it weight the informative cells.¶Because attention and pooling ignore order, shuffling the bag can't change the answer; and although it's trained only on fixed 100-cell bags, the architecture accepts any number of cells, so it can score a bag of ten or of thousands.¶The output is a probability over 1,000 perturbations (+NTC), or over 99 protein complexes when we ask about pathways instead of single geneKOs.",
  "Top-Predictive Cells": "Once the classifier works, we ask which individual cells it actually relies on, borrowing an idea from model explanation called \"explaining by removing\": a cell is important if taking it out of a bag makes the classifier less sure of the right answer. Concretely we take the probability of the correct class with the cell in the bag minus the probability without it, and average that marginal contribution over many random bags and bag sizes (a Monte-Carlo estimate).¶The highest-scoring cells are the clearest examples of a perturbation's phenotype — these top-predictive cells are exactly what the Top-Predictive Cells tab shows and what the diffusion traversals are anchored to. Up-weighting them also sharpens the perturbation-level distinctiveness score (mAP).",
...
const METHODS_SLIDES = [
  ...
  },
  {
    ...
    body: "Because single cells are noisy, we show the model a whole <b>group</b> of a perturbation's cells at once — each as its <b>CellDINO feature vector</b> (from the Embedding tab), not the raw image — and ask it to name the perturbation. It weighs the cells against each other, pools them into one verdict, and outputs a probability over the <b>1,000 perturbations</b> (or 99 protein complexes). It does this with a <b>SetTransformer</b> — the architecture is diagrammed below.",
    why: "This bag-level probability is the backbone of the whole atlas — every distinctiveness ranking, top-cell pick, and traversal target is derived from how confidently the model can name a perturbation from its cells.",
  ...
  {
    nav: "Top-Predictive Cells", kicker: "EXPLAINING BY REMOVING", title: "Which cells carry the phenotype?",
    ...
    body: "To find a perturbation's most telling cells, we score each cell by how much it <b>helps the classifier</b>: the drop in predicted probability when the cell is <b>removed</b> from a bag (\"explaining by removing\"), averaged over many bag sizes and random partners. The top-scoring <b>top-predictive cells</b> carry the phenotypic signature — the cells the viewer shows in Top-Predictive Cells and morphs toward in Image Generation.",
    ...
    defs: [["Explaining by removing", "gauge a cell's importance by how much the prediction drops when you take it out of the bag."],
      ...
      ["Top-predictive cells", "the highest-scoring cells for a class; used as traversal anchors and in the Top-Predictive Cells tab."],
  ...
  {
    nav: "Image Generation", kicker: "COUNTERFACTUAL", title: "\"How would this cell look if we applied a given perturbation?\"",
    ...
    body: "Each cell is split into two parts — a <b>detail code</b> that fixes this particular cell's exact pixel rendering and imaging context, and its <b>phenotype</b> (the CellDINO semantic code). The <b>destination</b> we morph toward is the <b>centroid</b> — the average semantic code — of the perturbation's <b>top cells</b> (the very cells in the Top-Predictive Cells tab). We hold the detail code <b>fixed</b> and slide only the semantic phenotype along the <b>NTC → knockout direction</b> (from the control centroid toward that top-cell centroid), decoding each step: α = 0 is the real cell, and larger α pushes further along that trajectory. Because the detail code stays locked, the cell's individual rendering and surroundings stay constant while its phenotype is what moves.",
...
const MTH_ORDER = ["The screen", "Embedding", "Classifier", "Top-Predictive Cells", "Diffusion", "Image Generation", "Montage",
...
// public deck = About + steps 1–6 (Screen…Image Generation); internal = full deck. isPublic() from app.js (loaded first).
...
function renderMethods() {
  ...
  if (rail && rail.dataset.decklen != deck.length) {   // (re)build when the deck size changes (e.g. public toggle)
    ...
    let _cn = 0;   // number method slides 1..N (About/Extra excluded) so rail matches the "n / N" kicker
    const _railBtns = deck.map((s, i) => { const x = MTH_EXTRA.has(s.nav);
      const num = s.about ? "•" : (x ? "+" : String(++_cn));
      return `<button class="mth-railitem${x ? " mth-railitem-x" : ""}" onclick="methodsGo(${i})"><span class="mth-num">${num}</span>${x ? "Extra · " + s.nav : s.nav}</button>`; }).join("");
    rail.innerHTML = `${single ? "" : `<div class="hint">A visual tour of the methods behind this viewer — click through, ← → to navigate.</div>
      <div class="mth-rail">${_railBtns}</div>`}
      <div class="mth-rail-foot">
        <a class="mth-paper" href="https://www.biorxiv.org/content/10.64898/2026.06.01.728087v1" target="_blank" rel="noopener">
  ...
  if (s.about) {   // About = first section: reuse the About panel content (source kept hidden in the DOM)
    ...
    view.innerHTML = `<div class="mth-card">
      ...
      ${MTH_MORE[s.nav] ? `<details class="mth-more"><summary>Learn more</summary>${MTH_MORE[s.nav].split("¶").map(p => `<p>${p.trim()}</p>`).join("")}</details>` : ""}
