...
def save_guide_eval(
    ...
    distinctiveness_map.to_csv(
        output_dir / "phenotypic_distinctiveness.csv", index=False
    )
    ...
    plot_map_scatter(
        ...
        distinctiveness_map,
        "Distinctiveness",
        metrics["pct_perturbations_distinct"],
    )
    fig.tight_layout()
    fig.savefig(
        output_dir / "map_activity_distinctiveness.png", dpi=150, bbox_inches="tight"
    )
    plt.close(fig)
...
def save_gene_eval(
    ...
    consistency_corum_map.to_csv(
        output_dir / "phenotypic_consistency_corum.csv", index=False
    )
    consistency_manual_map.to_csv(
        output_dir / "phenotypic_consistency_manual.csv", index=False
    )

    corum_entity_col = (
        "complex_id"
        if "complex_id" in consistency_corum_map.columns
        else consistency_corum_map.columns[0]
    )
    chad_entity_col = (
        "complex_num"
        if "complex_num" in consistency_manual_map.columns
        else consistency_manual_map.columns[0]
    )
    ...
    plot_map_scatter(
        ...
        "Consistency (CORUM)",
        metrics["pct_complexes_significant_corum"],
    )
    plot_map_scatter(
        ax2,
        consistency_manual_map,
        "Consistency (CHAD)",
        metrics["pct_complexes_significant_manual"],
    )
    fig.tight_layout()
