...
from ops_analysis.model.models import byol, cytoself_model, dynaclr, eval_mlp
...
def get_data_manager(config_path):
    ...
    data_manager = data_loader.OpsDataManager(
        ...
        data_split=tuple(config["evaluation"]["data_split"]),
    ...
    data_manager.construct_dataloaders(
        num_workers=config["data_manager"]["num_workers"], dataset_type=dataset_type
    )
...
def create_ann_data(embedding_path, num_samples, data_manager):
    ...
    adata.obs["experiment"] = [
        data_manager.labels_df.loc[a].store_key for a in entry_info.total_index
    ]
    adata.obs["tile"] = [
        data_manager.labels_df.loc[a].tile_pheno for a in entry_info.total_index
    ]
    adata.obs["x_local"] = [
        data_manager.labels_df.loc[a].x_local_pheno for a in entry_info.total_index
    ]
    adata.obs["y_local"] = [
        data_manager.labels_df.loc[a].y_local_pheno for a in entry_info.total_index
    ]
    adata.obs["gene_int"] = [a for a in entry_info.gene_label]
    adata.obsm["bbox"] = np.asarray(
        [data_manager.labels_df.loc[a].bbox for a in entry_info.total_index]
    )

    for i in np.random.randint(0, 1000, size=10):
        gene_string = adata.obs["gene_str"].iloc[i]
        gene_int = adata.obs["gene_int"].iloc[i]
        dm_info = data_manager.labels_df.loc[entry_info.loc[i].total_index]

        print("             aData    Data_manager")
        print(f"Gene String: {gene_string}, | {dm_info.gene_name}")
        print(
            f"Gene Int:    {gene_int},   | {data_manager.label_int_lut[dm_info.gene_name]}"
        )
        print("-" * 30)
...
def reduce_dims(adata, num_components, save_dir):
    ...
    ##### save a umap with a specific gene highlighted
    gene_of_interest = "NTC"
    unique_str_labels = np.unique(adata.obs.gene_str)
    color_dict = {gene: "grey" for gene in unique_str_labels}
    color_dict[gene_of_interest] = "blue"
    sc.pl.umap(
        adata,
        color="gene_str",
        palette=color_dict,
...
def mean_class_similarity(
    ...
    gene_labels = adata.obs["gene_str"].unique().tolist()
    similarities = {}
    for g in gene_labels:
        embeddings = torch.tensor(adata[adata.obs["gene_str"] == g].X).cuda()
...
def evaluate(config_path, num_samples: int = 1000, run_predict: bool = False):
    ...
    save_dir = Path(
        f"/hpc/projects/intracellular_dashboard/ops/models/predictions/{config['model_type']}/analyzed/{config['run_name']}"
    )

    if run_predict:
        embedding_path = run_inference(config, data_manager=data_manager)
        adata = create_ann_data(
            embedding_path=f"/hpc/projects/intracellular_dashboard/ops/models/predictions/{config['model_type']}/features_{config['run_name']}.zarr",
...
def evaluate_cli(config_path, num_samples, run_predict):
    return evaluate(config_path, num_samples, run_predict)
